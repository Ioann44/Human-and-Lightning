...
def jumper():
    global dog, shot, running, conn, hart, hart_n
            ...
            if event.key == pygame.K_LEFT:
                ...
                for d in dog.onGround:
                    if d:
                        ...
                        break
    taked_message = read((conn.recv(1024)),3)
    shot.shooting, shot.rect.left, shot.rect.top = taked_message
    message = str(dog.rect.left)+' '+str(dog.rect.top)+' '+\
              str(dog.x_vel)+' '+str(dog.y_vel)+' '
    if hart:
        message = message+str(hart_n)+' '
    else:
        message = message+'4 '
    conn.send(message)

def shooter():
    global shot, dog, running, conn, hart, hart_n
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            running = False
            going = False
    ...
    taked_message = read((conn.recv(1024)),4)
    dog.rect.left, dog.rect.top ,dog.x_vel, dog.y_vel = taked_message

def game():
    global game_mode,l,hart,platforms,hart_n,running
    running = True
